...
public partial class Admin_SelectStore : Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        foreach (UltraGridRow row in grdStore.Rows)
        {
            if (row.Cells.FromKey("Selected").Text == "true")
            {
                ...
                if (Request.QueryString["rid"] != null)
                {
                    if ((row.Cells.FromKey("KhuVucID").Text != Request.QueryString["rid"]) ||
                        (row.Cells.FromKey("ViTriCuaHang").Text != sid))
                    {
    ...
    private int LoadCuaHang(int ViTriCuaHang)
    {
        if (Common.LoaiNguoiDungID() == 3)
        {
            ...
            DataRow[] selectedRows = ds.Tables[0].Select("ViTriCuaHang=" + ViTriCuaHang);
    ...
    protected void grdStore_InitializeLayout(object sender, LayoutEventArgs e)
    {
