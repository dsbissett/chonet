...
if(!ig_WebControl.prototype.isPrototypeOf(ig_WebCalendarView.prototype))
{
	...
	this._setActiveDay=function(info,evt,day,id, smartCallback)
	{
		...
		{
			var activeDay = me._info.getActiveDay();
			if((activeDay.getMonth() == day.getMonth() && activeDay.getFullYear() == day.getFullYear()))
			...
			else
			{
				if(smartCallback != null)
				{
					...
					me._registerSmartCB(smartCallback, "ActiveDaySync", "html");
					me._registerSmartCB(smartCallback, "ActiveDaySync", "json");
					me._registerSmartCB(smartCallback, "ActiveDaySync", "styles");
					
					for(var i = 2; i < registeredControls.length; i++)
						smartCallback._registeredControls.push(registeredControls[i]);
					
					evt.needPostBack = true;
				}
			}
		}
		else
	...
	ig_shared.addEventListener(elem, "keyup", ig_handleEvent);
	...
	ig_WebCalendarView.prototype._onKeydown = function(src, evt)
	{
		...
		{			
			var elem = null;
			var attr = src.getAttribute("id_");
			if(src != this._element && (attr != null && attr < 500))
				src.tabIndex = -1; 
			
			if(attr != null && attr < 500)
			...
			else if(attr == null && evt.keyCode != 9)
				elem = this.Days[this._now[2] +1].element; 
