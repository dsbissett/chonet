...
function igdrp_combo(elem,props,ce)
{
	...
	{
		if(!igdrp_all._loaded&&ig_csom.IsIEWin)return;
		var evt,iePan=this._ieBug,pan=this.container,edit=this.Element;
		...
		this.focus();
		var cal=this.Calendar,editH=edit.offsetHeight,editW=edit.offsetWidth,e=edit,x=pan.parentNode,body=window.document.body;
		...
		if(!iePan&&this.HideDropDowns&&ig_csom.IsIEWin)
	...
	{
		var e=this.editor;
		var v=e?e.getDate():null,o=this.old;
	...
	this.onImgKey=function(evt)
	{
		...
		if(k==27||(this.Dropped&&(k==32||k==13)))
		{
...
{
	var pan=null,drop=false,me=igdrp_all[id],elem=igdrp_getElementById(id);
	if(me&&me.doEvt&&elem)
	{
		drop=me.Dropped;
		pan=me._ieBug;
		...
		if(m)m.tr=m.elem=m.parent=m.Event=null;
		me._ieBug=me.postField=me.elemCal=me.container=me.Element=me.inputBox=me.Calendar=me.webGrid=null;
		ig_dispose(me);
	}
	...
	{
		this.Element.value=d?(d.getFullYear()+"-"+(d.getMonth()+1)+"-"+d.getDate()):"";
		if(fire)fire=!this._noFire;
		if(!this._same(this.old,d))
		{
			this.update("Value",d);
			if(fire)this._msV_=window.event==null;
		}
		else if(!this._msV_)return;
		if(fire&&this.inputBox.onchange&&window.event)this.inputBox.onchange();
	}
...
function igmask_date(me,e,di,v,lf,nullTxt,nullable)
{
	...
	this.fireEvt=function(id,e)
	{
		...
		if(id==1)
		{
			...
			if(k==27||k==13){me.setDropDownVisible(false);me.updateValue();}
			me._fire(5,k,evt);
		...
		if(id==9)
		{
			if(this.changed)if(me.Dropped)me.changed=true;else me.updateValue();
			if(!me.Dropped)
			{
