...
{
	var o=igcal_all[id],elem=document.getElementById("igcal"+id);
	if(o)
	{
		var e0=o.element;
		if(elem!=e0)
	...
	var evs=document.getElementById(id);
	if(o&&evs)
	{
...
function igcal_new(elem,id,dates,prop,str)
{
	...
	for(i=12;i<18;i++)this.css[i-12]=this.valI(prop0,i);
	...
	this.repaint=function(year,month,check,e)
	{
		...
		for(i=0;i<42;i++)
		{
			...
			o.css=this.css[0];o.hide=this._hide;
			if(i<day1)
			{
				o.day=day0+i;
			...
			var text=o.hide?" ":o.day,sel=!o.hide&&this.isSelected(o.year,o.month,o.day,i);
			d=o.css;
			if(sel)d+=' '+this.css[4];
			if(id.length>0)
			{
