...
function iged_new(id,ta,tb,p1,p2,p3,p4,p5)
{
	...
	this.focus=function(){if(!this._foc)try{if(this._ie)this._elem.focus();else this._win().focus();}catch(i){}}
	...
	{
		if(!e||!e.getAttribute||e._oldP)return;
		e._oldP=true;
	...
	this._setOl=function()
	{
		...
		{
			iged_all._curRange=this._range();
			sel=iged_all._curRange.parentElement();
		}
		else sel=this._cont();
	...
	this._pos=function(evt,pan,flag)
	{
		...
		var pe,f,z,x=0,y=0,x0=0,y0=0,e=(flag==8)?evt:evt.target;
		...
		if(!elemH)elemH=20;
		var htm=null,ieBug=false,s=null,bp=body.parentNode,panW=80,panH=80;
		...
		y+=elemH;ok=true;pe=this._exp;
		var iH=body.clientHeight,iW=body.clientWidth,iL=body.scrollLeft,iT=body.scrollTop;
		if(!iH||iH<50){iH=body.offsetHeight;iW=body.offsetWidth;}
		z=body;
		while(!htm&&(z=z.parentNode)!=null)if(z.nodeName=='HTML')htm=z;
		if(htm)
		{
			z=htm.clientHeight;i=htm.offsetHeight;
			if(z&&z>20&&!this._opr)
			{iH=z;iW=htm.clientWidth;iL=htm.scrollLeft;iT=htm.scrollTop;}
		}
		if(iH<20)iH=20;if(iW<90)iW=90;
		if(iH<y-iT+panH){if(y-iT-3>panH+elemH){y-=panH+elemH;ok=!pe||(pe&3)==0;}else y=iH+iT-panH;}
		if(y<iT)y=iT;
		if(x+panW>iW+iL){x=iW+iL-panW;if(x<iL)x=iL;}
		if(x<iL)x=iL;
		if(this._mac&&(this._ie||this._saf)){x+=this._ie?5:-5;y+=this._ie?11:-7;}
		if(s){s.left=x+"px";s.top=(ok?y:(y+panH))+"px";}
		if(ieBug)this._ieBug(pan,x,y);
		if(pan&&pe)
		{
			z=iged_all._ip={i:0,f:pe,pan:pan,overF:s.overflow,oldH:s.height,y:(ok?-1:y),fn:window.setInterval("iged_popTimer()",15)};
			if((pe&3)!=0){z.h=1;z.panH=panH;s.overflow="hidden";s.height="1px";}
			if((pe&12)!=0){z.o=0.01;s.opacity='0.01';s.filter='progid:DXImageTransform.Microsoft.Alpha(opacity=1)';}
		}
	}
	this._move=function(e,par){try{var p=e.parentNode;p.removeChild(e);par.insertBefore(e,par.firstChild);e._moved=p;return true;}catch(e){}return false;}
	...
	{
		v=iged_replaceS(v,"%3C","<",s);
		v=iged_replaceS(v,"%3E",">",s);
		return iged_replaceS(v,"%22","\"",s);
	}
	this._update=function()
	{
		if(this._mod||this._foc)this._ta.value=this._fixStr(this._decode(this.getText()),true);
		this._mod=false;
	}
...
function iged_mainEvt(e)
{
	...
	if(t==6){o._update(e);o._foc=false;return;}
	if(o._ie)try
	{
	...
	if(t==5)o._mod=o._foc=true;
	if(t==8&&!inElem&&!o._foc&&!(src.unselectable&&iged_all._cur==o))window.setTimeout("iged_all._cur.focus()",0);
	iged_all._cur=o;
}
...
function iged_closePop(s)
{
	...
	if(o)if(o._fire(1,"ClosePopup",s))if(s)return;
	if(p._ieBug){p._ieBug.display="none";p._ieBug=null;}
	p.style.visibility="hidden";
	if(o)o._fire(2,"ClosePopup",s);
	iged_all._canCloseCur=false;
	if(o&&foc)o.focus();
	var p0=p._moved,p2=p.parentNode;
	if(p0&&p2){p2.removeChild(p);p0.appendChild(p);p._moved=null;}
	if(p._div)p.innerHTML="";
}
