...
function iged_init(ids,p1,p2,p3,p4,p5)
{
	...
	o._find=function(txt,flag,txt2)
	{
		...
		if(fr.findText(txt,this._findLen,flag))try{fr.select();this._findNum++;return true;}catch(s){}
		txt="\""+txt+"\"";s=15;
	...
	o._txt=function(txt,paste)
	{
		...
		while(true)
		{
			...
			for(i=0;i<n.length;i++)
			{
				...
				try{if((nx=vv[parseInt(nx)])!=null)if(x.length==1)n[i].href=nx;else n[i].src=nx;}
				catch(ii){}
	...
	o._ieBug=function(pan,x,y)
	{
		...
		if(!f)
		{
			f=d.createElement("IFRAME");f.style.display="none";f.src="javascript:''";
			d.body.insertBefore(f,d.body.firstChild);f.frameBorder=f.scrolling="no";
		}
		var s=f.style;
	...
	o.setText(o._fixStr(ta.value),1);
	var edit=o._prop[12];
	if(edit>0&&!elem._oldE)
	{
		elem._oldE=true;
