...
function _igtbl_determineEditorPosition(g,gn,cellObj,cElem,cell)
{
	...
	{
	    if (cElem.currentStyle)
	    {
	        
	        var borderLeftWidth = parseInt(cElem.currentStyle.borderLeftWidth);
	        if(typeof(borderLeft) != "undefined")
	            r.x +=  Math.floor(borderLeftWidth/2);
	        
	        var borderTopWidth = parseInt(cElem.currentStyle.borderTopWidth);
	        if(typeof(borderTopWidth) != "undefined")
	            r.y +=  Math.floor(borderTopWidth/2);
	    }   
	}
...
function _igtbl_textareaEditorKeyPress(evnt) {
    ...
    var column = igtbl_getCellById(textbox.currentCell).Column; 
