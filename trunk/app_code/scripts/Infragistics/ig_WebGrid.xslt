	...
	<xsl:if test="$useFixedHeaders">
		<td colspan="{$band/@nfspan}">
			...
			<div id="{$gridName}_drs">
				<xsl:attribute name="style">
					<xsl:choose>
						<xsl:when test="$row/@dtdh">overflow:hidden;<xsl:if test="$isXhtml">position:relative;</xsl:if></xsl:when>
						<xsl:otherwise>overflow:hidden;width:100%;height:100%;<xsl:if test="$isXhtml">position:relative;</xsl:if></xsl:otherwise>
					</xsl:choose>
				</xsl:attribute>
				<table border="0" cellspacing="0" cellpadding="0" style="position:relative;table-layout:fixed;{$fixedScrollLeft}">
					<colgroup>
						<xsl:for-each select="$band/Columns/Column">
...
<xsl:template match="Group">
	...
	<tr id="{$gridName}_gr_{$parentRowLevel}{$rowIndex}" groupRow="{@groupRow}" level="{$parentRowLevel}{$rowIndex}">
		...
		<td>
			<table height='100%' border='0' cellpadding='0' cellspacing='0' bgcolor='{@bgcolor}' bandNo='{@bandNo}'>
				...
				<tr id="{$gridName}_sgr_{$parentRowLevel}{$rowIndex}" level="{$parentRowLevel}{$rowIndex}" groupRow="{@groupRow}">
					<td id="{$gridName}_grc_{$parentRowLevel}{$rowIndex}" groupRow="{@groupRow}" class='{$band/@grpClass}'>
